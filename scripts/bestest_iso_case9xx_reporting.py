...
import pandas as pd
from bestest_iso_case6xx_reporting import annual_tracks, comparison_table, peak_tracks, range_judgement

ROOT = Path(__file__).resolve().parents[1]

def prescribed_changes(case: str) -> pd.DataFrame:
    data = json.loads((ROOT / "inputs" / "1_iso" / "case9xx_deltas.json").read_text())
