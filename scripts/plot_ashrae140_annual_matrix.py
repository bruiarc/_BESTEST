...
from bestest_reporting import markdown_table
...
def _actual(case: str, implementation: str, mode: str, metric: str) -> float:
    data = pd.read_csv(ROOT / "results" / f"case{case}" / f"case{case}_metrics.csv")
...
def main() -> None:
    ...
    output = ROOT / "results" / "ashrae140_annual_load_matrix.csv"
    summary.to_csv(output, index=False)
    figure, axes = plt.subplots(2, 1, figsize=(15, 15.5))
    draw_table(axes[0], "heating", summary); draw_table(axes[1], "cooling", summary)
    figure.tight_layout()
    figure.savefig(ROOT / "results" / "ashrae140_annual_load_matrix.png", dpi=180, bbox_inches="tight")
